...
def _search_cachekey(method, self, *args, **kwargs):
    ...
    key = [timeout] + [iam] + [arg for arg in args] + [kwarg for kwarg in kwargs.items()]
    #print str(key)
    return str(key)

@ram.cache(_search_cachekey)
def search_ext(self,base,scope,filterstr='(objectClass=*)',attrlist=None,attrsonly=0,serverctrls=None,clientctrls=None,timeout=-1,sizelimit=0):
    ret = orig_search_ext(self,base,scope,filterstr,attrlist,attrsonly,serverctrls,clientctrls,timeout,sizelimit)
    return ret
...
def search_ext_s(self,base,scope,filterstr='(objectClass=*)',attrlist=None,attrsonly=0,serverctrls=None,clientctrls=None,timeout=-1,sizelimit=0):
    msgid = orig_search_ext(self,base,scope,filterstr,attrlist,attrsonly,serverctrls,clientctrls,timeout,sizelimit)
    return self.result(msgid,all=1,timeout=timeout)[1]

print 'REBINDING python-ldap search_ext for caching'
